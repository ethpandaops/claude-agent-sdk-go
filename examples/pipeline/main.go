...
func getAssistantText(
	...
	for msg, err := range msgs {
		...
		if m, ok := msg.(*claudesdk.AssistantMessage); ok {
...
func generateEvaluateRefine() {
	...
	draft, cost, err := getAssistantText(ctx, claudesdk.Query(ctx,
		"Write a short product description (2-3 sentences) for a smart water bottle "+
			"that tracks hydration and syncs with a phone app.",
		claudesdk.WithSystemPrompt("You are a marketing copywriter. Write concise, compelling copy."),
	...
	evaluation, cost, err := getAssistantText(ctx, claudesdk.Query(ctx,
		evaluatePrompt,
		claudesdk.WithSystemPrompt(
	...
	refined, cost, err := getAssistantText(ctx, claudesdk.Query(ctx,
		refinePrompt,
		claudesdk.WithSystemPrompt("You are a marketing copywriter. Improve copy based on feedback."),
		claudesdk.WithMaxTurns(1),
	))
	if err != nil {
