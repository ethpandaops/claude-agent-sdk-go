...
// Claude SDK. Since the SDK expects *slog.Logger, this example shows how to create
...
func displayMessage(log logrus.FieldLogger, msg claudesdk.Message) {
	switch m := msg.(type) {
...
func main() {
	...
	for msg, err := range claudesdk.Query(ctx, "What is 2+2? Answer in one short sentence.",
		claudesdk.WithLogger(slogLogger),
