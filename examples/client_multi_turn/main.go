...
func exampleManualHandling() {
	...
	for msg, err := range client.ReceiveMessages(ctx) {
		...
		if resultMsg, ok := msg.(*claudesdk.ResultMessage); ok {
...
func exampleWithOptions() {
	...
	for msg, err := range client.ReceiveResponse(ctx) {
		...
		if assistantMsg, ok := msg.(*claudesdk.AssistantMessage); ok {
			...
			for _, block := range assistantMsg.Content {
				if toolUseBlock, ok := block.(*claudesdk.ToolUseBlock); ok {
	...
	for msg, err := range client.ReceiveMessages(ctx) {
		...
		case *claudesdk.UserMessage:
			for _, block := range m.Content.Blocks() {
				if textBlock, ok := block.(*claudesdk.TextBlock); ok {
					fmt.Printf("User: %s\n", textBlock.Text)
				} else if toolResultBlock, ok := block.(*claudesdk.ToolResultBlock); ok {
					// Extract text from ContentBlocks
					var contentText string

					for _, cb := range toolResultBlock.Content {
						if tb, ok := cb.(*claudesdk.TextBlock); ok {
							contentText += tb.Text
						}
					}
		...
		case *claudesdk.AssistantMessage:
			for _, block := range m.Content {
				if textBlock, ok := block.(*claudesdk.TextBlock); ok {
					fmt.Printf("Claude: %s\n", textBlock.Text)
				} else if toolUseBlock, ok := block.(*claudesdk.ToolUseBlock); ok {
...
func exampleErrorHandling() {
	...
	for msg, err := range client.ReceiveMessages(shortCtx) {
		...
		} else if _, ok := msg.(*claudesdk.ResultMessage); ok {
			displayMessage(msg)
...
func main() {
	...
	if exampleName == "all" {
		...
		exampleOrder := []string{
			...
			"with_interrupt", "manual_message_handling", "with_options",
			"async_iterable_prompt", "bash_command", "control_protocol", "error_handling",
		}
