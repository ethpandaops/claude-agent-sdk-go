...
func createMemoryTools(store *MemoryStore) []*claudesdk.SdkMcpTool {
	// Read tool
	readTool := claudesdk.NewSdkMcpTool(
		"read",
		"Read file contents or list directory. Path must start with /memories",
		claudesdk.SimpleSchema(map[string]string{"path": "string"}),
		func(_ context.Context, req *claudesdk.CallToolRequest) (*claudesdk.CallToolResult, error) {
			...
			return claudesdk.TextResult(content), nil
		},
	)

	// Write tool
	writeTool := claudesdk.NewSdkMcpTool(
		"write",
		"Create or overwrite a file. Path must start with /memories",
		claudesdk.SimpleSchema(map[string]string{"path": "string", "content": "string"}),
		func(_ context.Context, req *claudesdk.CallToolRequest) (*claudesdk.CallToolResult, error) {
			args, err := claudesdk.ParseArguments(req)
			if err != nil {
				return claudesdk.ErrorResult(err.Error()), nil
			...
			return claudesdk.TextResult(fmt.Sprintf("Successfully wrote to %s", path)), nil
		},
	)

	// Append tool
	appendTool := claudesdk.NewSdkMcpTool(
		"append",
		...
		func(_ context.Context, req *claudesdk.CallToolRequest) (*claudesdk.CallToolResult, error) {
			...
			return claudesdk.TextResult(fmt.Sprintf("Successfully appended to %s", path)), nil
		},
	)

	// Delete tool
	deleteTool := claudesdk.NewSdkMcpTool(
		"delete",
		...
		func(_ context.Context, req *claudesdk.CallToolRequest) (*claudesdk.CallToolResult, error) {
			...
			return claudesdk.TextResult(fmt.Sprintf("Successfully deleted %s", path)), nil
		},
	)

	// Rename tool
	renameTool := claudesdk.NewSdkMcpTool(
		"rename",
		"Rename or move a file/directory. Paths must start with /memories",
		claudesdk.SimpleSchema(map[string]string{"old_path": "string", "new_path": "string"}),
		func(_ context.Context, req *claudesdk.CallToolRequest) (*claudesdk.CallToolResult, error) {
			args, err := claudesdk.ParseArguments(req)
			if err != nil {
				return claudesdk.ErrorResult(err.Error()), nil
			...
			return claudesdk.TextResult(fmt.Sprintf("Successfully renamed %s to %s", oldPath, newPath)), nil
		},
	)

	// List tool
	listTool := claudesdk.NewSdkMcpTool(
		"list",
		...
		func(_ context.Context, req *claudesdk.CallToolRequest) (*claudesdk.CallToolResult, error) {
			...
			if len(entries) == 0 {
				return claudesdk.TextResult("Directory is empty"), nil
			}

			return claudesdk.TextResult(strings.Join(entries, "\n")), nil
		},
	)

	return []*claudesdk.SdkMcpTool{readTool, writeTool, appendTool, deleteTool, renameTool, listTool}
}
...
func main() {
	...
	fmt.Println("Demonstrating filesystem-backed persistent memory for Claude")
	...
	memoryServer := claudesdk.CreateSdkMcpServer("memory", "1.0.0", tools...)
	...
	if startErr := client.Start(ctx,
		...
		claudesdk.WithSystemPrompt("You have access to memory tools that persist data to the filesystem. "+
