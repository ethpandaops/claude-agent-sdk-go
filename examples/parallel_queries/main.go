...
func parallelTranslations() {
	...
	for _, style := range styles {
		g.Go(func() error {
			text, cost, err := getAssistantText(gCtx, claudesdk.Query(gCtx,
				fmt.Sprintf("Translate this to French: %q", originalText),
				claudesdk.WithSystemPrompt(style.Prompt),
	...
	judgeText, judgeCost, err := getAssistantText(ctx, claudesdk.Query(ctx,
		judgePrompt,
